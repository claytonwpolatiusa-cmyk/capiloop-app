...
const options = [
  ...
  { icon: "forum", title: "Meus chamados", detail: "Acompanhe seu atendimento" },
...
export default function ProfileScreen() {
  ...
  const adminAccess = trpc.adminSupport.access.useQuery(undefined, { enabled: isAuthenticated });
  const profileOptions = adminAccess.data?.allowed
    ? [...options, { icon: "admin-panel-settings", title: "Atendimento CapiLoop", detail: "Gerencie chamados da equipe" }, { icon: "verified-user", title: "Aprovação de parceiros", detail: "Revise estabelecimentos pendentes" }]
    : options;
  ...
  return <ScreenContainer edges={["top", "left", "right"]} className="flex-1"><FlatList
    data={profileOptions} keyExtractor={(item) => item.title} contentContainerStyle={[styles.content, isCompact && styles.contentCompact]}
    renderItem={({ item }) => <Pressable accessibilityRole="button" onPress={() => item.title === "Central de ajuda" ? router.push("/help" as never) : item.title === "Meus chamados" ? router.push("/support-tickets" as never) : item.title === "Atendimento CapiLoop" ? router.push("/admin/support" as never) : item.title === "Aprovação de parceiros" ? router.push("/admin/partners" as never) : item.title === "Favoritos" ? router.push("/favorites" as never) : item.title === "Histórico de pedidos" ? router.push("/order-history" as never) : undefined} style={({ pressed }) => [styles.option, pressed && { opacity: 0.65 }]}><View style={styles.optionIcon}><MaterialIcons name={item.icon as never} size={20} color="#151B14" /></View><View style={styles.optionInfo}><Text style={styles.optionTitle}>{item.title}</Text><Text style={styles.optionDetail}>{item.detail}</Text></View><MaterialIcons name="chevron-right" size={23} color="#AEB5AA" /></Pressable>}
